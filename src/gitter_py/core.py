...
def gitter(
    ...
    if autorotate:
        raise ValueError(
            "autorotate is no longer supported because the PET dependency is not maintained."
        )
    ...
    if is_ref:
        if start_coords is not None and increment_coords is not None:
            ...
            cp_y = colony_peaks(sum_y, n=plate_rows, plate_size=plate_rows * plate_cols, plot=plot)
            cp_x = colony_peaks(sum_x, n=plate_cols, plate_size=plate_rows * plate_cols, plot=plot)
    ...
    **kwargs: Any,
    ...
    for image_file in image_list:
        try:
            ...
            failed.append(Path(image_file).name)
            if verbose == "p":
                print(f"Failed to process {image_file}, skipping")
